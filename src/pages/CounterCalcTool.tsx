...
function runMonteCarlo(boss: BossState, slots: TeamSlot[], bossHP: number, opts: MCOptions): MCResult | null {
  ...
  // All raiders use the same team — expand slots × raiders
  ...
  return (
    <div style={{ ...CARD, padding: '14px', position: 'relative' }}>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-start' }}>
        ...
        {/* Action buttons */}
        <button onClick={onDuplicate} style={{ ...BTN('ghost'), padding: '5px 8px', marginTop: 18 }} title="Duplicate slot">📋</button>
        <button onClick={onRemove} style={{ ...BTN('danger'), padding: '5px 8px', marginTop: 18 }} title="Remove">✕</button>
      </div>
      ...
      {showEvIv && (
        <div style={{ marginTop: 10, padding: '10px 12px', background: 'rgba(0,0,0,.2)', borderRadius: 8 }}>
          ...
          <div style={{ fontSize: 10, color: evTotal > 510 ? '#f87171' : 'var(--text-muted)', marginTop: 6 }}>
            Total EVs: {evTotal}/510{evTotal > 510 ? ' ⚠ Exceeds limit' : ''}
          </div>
        </div>
      )}
...
// Pits each counter sequentially against the boss (using average damage values).
// Each counter fights until it faints or the boss dies. Returns cumulative stats.
// ----------------------------------------------------------------------

interface BattleSlotResult {
  name: string;
  hitsDealt: number;
  dmgDealt: number;
  dmgTaken: number;
...
interface TeamBattleResult {
  ...
  remainingBossHpPct: number;
  perSlot: BattleSlotResult[];
}
...
function simulateTeamBattle(
  ...
  if (!boss.data || totalHp <= 0) {
    return { totalDmgDealt: 0, totalDmgPct: 0, bossKilled: false, remainingBossHp: totalHp, remainingBossHpPct: 100, perSlot: [] };
  }

  const bossMoves = getLevelUpMoves(boss.name);
  let bossHp = totalHp;
  const perSlot: BattleSlotResult[] = [];

  for (const slot of slots) {
    if (bossHp <= 0) break;
    if (!slot.data || !slot.result || (slot.result as any).immune) continue;
    const r = slot.result as any;

    // Counter HP
    const counterMaxHp = calcStat(slot.data.stats.hp, slot.evs.hp, slot.ivs.hp, true, 1, slot.level || 100);
    const avgAtkDmgPerHit = Math.max(1, (r.minD + r.maxD) / 2);

    // Average boss damage per hit against this counter (mean across all boss moves)
    let avgBossDmgPerHit = 0;
    if (bossMoves.length) {
      let totalBD = 0, mvCount = 0;
      for (const mv of bossMoves) {
        const defRes = runCalc({
          atkPoke: boss.data!, defPoke: slot.data,
        ...
        if (defRes && !defRes.immune) {
          totalBD += ((defRes as any).minD + (defRes as any).maxD) / 2;
          mvCount++;
        }
      }
      avgBossDmgPerHit = mvCount > 0 ? totalBD / mvCount : 0;
    }

    // Speed: determine turn order
    const atkSpe  = calcStat(slot.data.stats.spe, slot.evs.spe, slot.ivs.spe, false, getNat(slot.nature, 'spe'), slot.level || 100);
    const bossSpe = calcStat(boss.data!.stats.spe, boss.evs.spe, boss.ivs.spe, false, getNat(boss.nature, 'spe'), boss.level || 100);
    const faster  = atkSpe >= bossSpe;
    ...
    while (hp > 0 && bossHp > 0) {
      if (faster) {
        const d = Math.min(avgAtkDmgPerHit, bossHp);
        bossHp  -= d;
        dmgDealt += d;
        hitsDealt++;
        if (bossHp <= 0) break;
      ...
      // Boss retaliates
      if (avgBossDmgPerHit > 0) {
        hp       -= avgBossDmgPerHit;
        dmgTaken += avgBossDmgPerHit;
      }
      if (!faster && hp > 0 && bossHp > 0) {
        const d = Math.min(avgAtkDmgPerHit, bossHp);
        bossHp  -= d;
        dmgDealt += d;
        hitsDealt++;
  ...
  const totalDmgDealt = perSlot.reduce((s, p) => s + p.dmgDealt, 0);
  return {
    totalDmgDealt,
    totalDmgPct:         totalHp > 0 ? (totalDmgDealt / totalHp) * 100 : 0,
    bossKilled:          bossHp <= 0,
    remainingBossHp:     Math.max(0, bossHp),
    remainingBossHpPct:  totalHp > 0 ? (Math.max(0, bossHp) / totalHp) * 100 : 0,
    perSlot,
  };
}
...
function ResultsPanel({ slots, boss, totalHp }: { slots: TeamSlot[]; boss: BossState; totalHp: number }) {
  ...
  return (
    <div style={{ ...CARD, border: `1px solid ${battle.bossKilled ? 'rgba(52,211,153,.3)' : 'rgba(248,113,113,.3)'}`,
      background: battle.bossKilled ? 'rgba(52,211,153,.04)' : 'rgba(248,113,113,.04)' }}>

      <div style={{ fontSize: 11, fontWeight: 800, color: winColor, textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 12 }}>
        {winLabel}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 10, marginBottom: 14 }}>
        ...
        <Stat label="Team Dealt" value={`${battle.totalDmgPct.toFixed(1)}%`}
          sub={`${Math.round(battle.totalDmgDealt).toLocaleString()} dmg`} color={winColor} />
        <Stat label="Boss HP Left" value={`${battle.remainingBossHpPct.toFixed(1)}%`}
          sub={battle.bossKilled ? 'Finished!' : `${Math.round(battle.remainingBossHp).toLocaleString()} HP`}
          color={battle.bossKilled ? '#34d399' : '#f87171'} />
        <Stat label="Raiders Needed" value={battle.bossKilled ? '1' : String(Math.ceil(100 / Math.max(battle.totalDmgPct, 0.1)))}
          color={battle.bossKilled ? '#34d399' : '#fbbf24'} />
      </div>

      {/* Per-counter battle breakdown */}
      <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase',
        letterSpacing: '.07em', marginBottom: 8 }}>🗡 Battle Order — Sequential Fight</div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
        {battle.perSlot.map((s, i) => {
          const pct = totalHp > 0 ? (s.dmgDealt / totalHp) * 100 : 0;
          return (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '18px 1fr auto auto auto', gap: 8,
              alignItems: 'center', padding: '7px 10px', background: 'rgba(0,0,0,.2)', borderRadius: 8 }}>
              {/* slot number */}
              <span style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 800 }}>#{i+1}</span>
              {/* name + damage bar */}
              <div>
                ...
                <div style={{ height: 5, background: 'rgba(255,255,255,.07)', borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${Math.min(100, pct)}%`,
                    background: pct >= 50 ? '#34d399' : pct >= 25 ? '#fbbf24' : 'var(--primary)', borderRadius: 3 }} />
                </div>
              </div>
              {/* damage dealt */}
              <span style={{ fontSize: 12, fontWeight: 800, color: '#fff', fontFamily: "'JetBrains Mono',monospace",
                textAlign: 'right', minWidth: 50 }}>{pct.toFixed(1)}%</span>
              {/* hits */}
              <span style={{ fontSize: 10, color: 'var(--text-muted)', minWidth: 46 }}>{s.hitsDealt} hits</span>
              {/* status badge */}
              <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 4,
                background: s.fainted ? 'rgba(248,113,113,.15)' : 'rgba(74,222,128,.15)',
                color: s.fainted ? '#f87171' : '#4ade80', whiteSpace: 'nowrap' }}>
                {s.fainted ? '💀 Fainted' : '✅ Survived'}
              </span>
            </div>
          );
        })}
      ...
      {!battle.bossKilled && (
        <div style={{ marginTop: 10, fontSize: 11, color: 'var(--text-muted)' }}>
          Boss has <strong style={{ color: '#f87171' }}>{battle.remainingBossHpPct.toFixed(1)}%</strong> HP remaining after your full team.
          {' '}Estimated <strong style={{ color: '#fbbf24' }}>{Math.ceil(100 / Math.max(battle.totalDmgPct, 0.1))}</strong> raiders needed to finish.
        </div>
      )}
    </div>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={CARD}>
        ...
        <div style={{ marginTop: 8, fontSize: 11, color: 'var(--text-muted)', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          ...
          <span style={{ color: '#818cf8', fontWeight: 700, background: 'rgba(129,140,248,.12)', padding: '2px 8px', borderRadius: 4 }}>
            → Use {autoCat} moves ({boss.data.stats.def < boss.data.stats.spd ? 'Def is weaker' : 'SpD is weaker'})
          </span>
          {bossTypes.length === 2 && (
            <span style={{ color: '#c084fc', fontWeight: 700, fontSize: 10, background: 'rgba(192,132,252,.1)', padding: '2px 8px', borderRadius: 4 }}>
              Dual-type → find moves weak to BOTH types for 4×
            </span>
          )}
        </div>
      </div>
      ...
      {activeTab === 'team' && (
        <>
          ...
          <div style={{ ...CARD, background: 'rgba(88,101,242,.04)', border: '1px solid rgba(88,101,242,.15)' }}>
            <div style={{ fontSize: 11, fontWeight: 800, color: '#818cf8', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 10 }}>
              💡 Tips for Picking Raid Teams
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6 }}>
              <div style={{ display: 'flex', gap: 8 }}>
                <span style={{ color: '#818cf8', fontWeight: 800, flexShrink: 0 }}>1️⃣</span>
                <div><strong style={{ color: 'var(--text)' }}>Check boss weaknesses first.</strong>{' '}
                  Use the Auto-Find tab or the weakness chart at the top. Look for <em>4× and 2× weaknesses</em> — those are your priority types.
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <span style={{ color: '#818cf8', fontWeight: 800, flexShrink: 0 }}>2️⃣</span>
                <div><strong style={{ color: 'var(--text)' }}>Match the move category to the boss's weaker defense.</strong>{' '}
                  If boss <strong>Def &lt; SpD</strong> → use <em>Physical</em> moves. If <strong>SpD &lt; Def</strong> → use <em>Special</em> moves.
                  The Auto-Find panel detects this automatically and highlights it.
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <span style={{ color: '#818cf8', fontWeight: 800, flexShrink: 0 }}>3️⃣</span>
                <div><strong style={{ color: 'var(--text)' }}>Use your counter's strongest move for that type.</strong>{' '}
                  Higher BP + STAB + super-effective = maximum output. STAB alone is 1.5× — combine it with a 2× weakness for 3× total.
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <span style={{ color: '#818cf8', fontWeight: 800, flexShrink: 0 }}>4️⃣</span>
                <div><strong style={{ color: 'var(--text)' }}>Review the boss's moveset.</strong>{' '}
                  A counter with high damage output is useless if the boss instantly KOs it. Look at OHKO Risk — keep counters below 30% OHKO risk.
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <span style={{ color: '#818cf8', fontWeight: 800, flexShrink: 0 }}>5️⃣</span>
                <div><strong style={{ color: 'var(--text)' }}>Dual-type bosses: check both types for weaknesses.</strong>{' '}
                  A type that's weak against <em>both</em> of the boss's types hits for 4×. Shadow moves are always 2× per type (4× on dual-type).
                </div>
              </div>
            </div>
          </div>
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {!R && !running && (
            <div style={{ fontSize: 11, color: '#374151', lineHeight: 1.65, padding: '10px 12px', background: 'rgba(0,0,0,0.18)', borderRadius: 8 }}>
              <strong style={{ color: '#6b7280' }}>How it works:</strong> Pilot run of 1 000 trials → estimates σ → computes N = (1.96σ/margin)² → continues until CI width ≤ target or max trials.
              Each of the {boss.numRaiders} raiders sends the same team sequentially vs {totalHp.toLocaleString()} total boss HP.
              Boss retaliates with level-up moves each turn. Faster Pokémon attack first.
          ...
          {R && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 13px', background: 'rgba(0,0,0,0.22)', borderRadius: 8, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: 10, color: '#374151', marginLeft: 'auto' }}>{R.trials.toLocaleString()} trials · {R.numRaiders}×{R.numSlots} slots · seed {R.seed}</span>
              ...
              {showDiag && (
                <div style={{ background: 'rgba(0,0,0,0.18)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 9, padding: '13px 15px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 7 }}>
                    {[['Pilot σ', R.pilotStdDev.toFixed(4)], ['Required N', R.trialsRequired.toLocaleString()], ['Actual N', R.trials.toLocaleString()], ['CI Width', R.ciWidth.toFixed(4)]].map(([l, v]) => (
              ...
              {activeTab === 'hist' && (
                <div style={{ background: 'rgba(0,0,0,0.22)', borderRadius: 9, padding: '14px 14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    ...
                    <span style={{ fontSize: 10, color: '#374151' }}>σ={R.stdDev.toFixed(3)} IQR=[{R.p25},{R.p75}]</span>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 14, marginTop: 8, fontSize: 9, color: '#374151', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ color: '#818cf8' }}>K={K} ({R.numRaiders}r×{R.numSlots}s)</span>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'patk' && (
                <div style={{ background: 'rgba(0,0,0,0.18)', borderRadius: 9, padding: '14px' }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8 }}>P(defeat boss with ≤ k attacker slots)</div>
                  {confK && <div style={{ fontSize: 11, color: '#a5b4fc', fontWeight: 700, marginBottom: 8 }}>≤{confK} slots at {((R.pAtMostK.find(x => x.k === confK)?.p ?? 0) * 100).toFixed(1)}%</div>}
                    ...
                    {R.pAtMostK.map(({ k, p }) => {
                      ...
                      return (
                        <div key={k} onClick={() => setConfK(confK === k ? null : k)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 9px', cursor: 'pointer', background: confK === k ? 'rgba(99,102,241,0.15)' : 'rgba(255,255,255,0.02)', borderRadius: 6, border: `1px solid ${confK === k ? 'rgba(99,102,241,0.5)' : 'rgba(255,255,255,0.05)'}` }}>
                          <span style={{ fontSize: 11, color: '#9ca3af', minWidth: 24, fontWeight: 700 }}>≤{k}</span>
