...
import {
  ...
  RefreshCw, ChevronDown, ChevronUp, Mic, MicOff,
} from 'lucide-react';
import {
...
function StatCard({label,value,icon:Icon,color}:{label:string;value:number;icon:any;color:string}) {
  return <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:'14px 18px',display:'flex',alignItems:'center',gap:12,flex:'1 1 160px',minWidth:140}}>
...
export default function ActivityPage({guildId}:Props):JSX.Element {
  ...
  const load=(lim=limit)=>{
    ...
    Promise.all([getLeaderboard(guildId,lim),getActivityStats(guildId)])
    ...
    <div className="animate-fade" style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,overflow:'hidden'}}>
        <div style={{padding:'14px 20px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',gap:10}}>
          ...
          <button onClick={()=>load()} style={{marginLeft:'auto',background:'none',border:'none',cursor:'pointer',color:'var(--text-faint)',padding:4}}><RefreshCw size={12}/></button>
        </div>
        {loading?(
                ...
                {members.map((m,i)=>(
                  <tr key={m.user_id} className="data-row" style={{borderBottom:'1px solid var(--border)'}}>
                    <td style={{padding:'10px 16px',width:60}}>
                      <span style={{fontSize:14}}>{TROPHY_ICONS[i]??<span className="mono" style={{fontSize:11,color:'var(--text-faint)'}}>#{i+1}</span>}</span>
                    </td>
                    <td style={{padding:'10px 16px'}}>
                      <div style={{display:'flex',alignItems:'center',gap:9}}>
                        {m.avatar_url?<img src={m.avatar_url} alt="" style={{width:28,height:28,borderRadius:'50%',flexShrink:0}}/>:<div style={{width:28,height:28,borderRadius:'50%',background:'var(--elevated)',flexShrink:0}}/>}
                    ...
                    <td style={{padding:'10px 16px',fontSize:12,color:'var(--text-muted)',whiteSpace:'nowrap'}}>{m.last_active?fmt(m.last_active):'—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <>
                {vcStats&&(
                  <div style={{display:'flex',gap:12,padding:'12px 20px',borderBottom:'1px solid var(--border)',flexWrap:'wrap'}}>
                    {[{l:'Total VC Time',v:fmtDur(vcStats.totalSecs),c:'#a78bfa'},{l:'Active (7d)',v:vcStats.active7d.toString(),c:'var(--success)'},{l:'Active (24h)',v:vcStats.active24h.toString(),c:'var(--warning)'}].map(({l,v,c})=>(
                ...
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>
                    {vcMembers.map((m,i)=>(
                      <tr key={m.user_id} className="data-row" style={{borderBottom:'1px solid var(--border)'}}>
                        <td style={{padding:'10px 16px',width:60}}>
                          {i<3?<span style={{fontSize:14}}>{TROPHY_ICONS[i]}</span>:<span className="mono" style={{fontSize:11,color:'var(--text-faint)'}}>#{i+1}</span>}
                        </td>
                        <td style={{padding:'10px 16px'}}>
                          <div style={{display:'flex',alignItems:'center',gap:9}}>
                            {m.avatar_url?<img src={m.avatar_url} alt="" style={{width:28,height:28,borderRadius:'50%',flexShrink:0}}/>:<div style={{width:28,height:28,borderRadius:'50%',background:'var(--elevated)',flexShrink:0}}/>}
                        ...
                        <td style={{padding:'10px 16px'}}><span style={{fontSize:13,fontFamily:"'JetBrains Mono',monospace",color:'#a78bfa',fontWeight:700}}>{fmtDur(m.total_seconds)}</span></td>
                        <td style={{padding:'10px 16px'}}><span className="mono" style={{fontSize:12,color:'var(--text-muted)'}}>{m.session_count}</span></td>
                        <td style={{padding:'10px 16px',fontSize:12,color:'var(--text-muted)'}}>{m.last_active?fmt(m.last_active):'—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
