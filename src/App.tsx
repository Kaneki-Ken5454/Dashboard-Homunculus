  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg)',padding:24}}>
      ...
      <div style={{width:'100%',maxWidth:420,position:'relative',zIndex:1}}>
        ...
        <div style={{textAlign:'center',marginTop:18,fontSize:11,color:'var(--text-faint)'}}>Homunculus · Discord OAuth 2.0</div>
      </div>
    </div>
  );
}
...
function AdminDashboard({user,onLogout,appearance}:{user:DiscordUser;onLogout:()=>void;appearance:AppearanceConfig}) {
  ...
  const discover=useCallback(()=>{
    ...
    discoverAllGuildIds().then(list=>{
      setDbGuilds(list);
      // Always auto-select top guild from DB (most data)
      if(list.length>0) setGuildId(list[0].guild_id);
    }).catch(e=>setDiscoverErr((e as Error).message)).finally(()=>setDiscovering(false));
  },[]);
  useEffect(()=>{discover();},[]);
  useEffect(()=>{
