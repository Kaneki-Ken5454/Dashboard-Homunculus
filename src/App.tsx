  ...
  const PageComp = () => {
    if (!guildId) return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', gap: 14, textAlign: 'center' }}>
        ...
        {discovering
          ? <div style={{ color: 'var(--text-muted)', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}><Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> Scanning database for guild IDs…</div>
          : guilds.length === 0
      ...
      <aside style={{
        ...
        <nav style={{ flex: 1, overflowY: 'auto', padding: '8px 8px' }}>
          {NAV.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setPage(id)} className={page === id ? 'nav-active' : ''}
              style={{
                width: '100%', display: 'flex', alignItems: 'center', gap: 9,
                padding: '9px 10px', borderRadius: 8, border: 'none', cursor: 'pointer',
                background: 'none', color: page === id ? undefined : 'var(--text-muted)',
                fontSize: 13, fontFamily: 'Lexend, sans-serif', fontWeight: 500,
        ...
        <div style={{ padding: '10px 10px 14px', borderTop: '1px solid var(--border)' }}>
              ...
              <div style={{
                ...
                <div style={{ position: 'relative', marginBottom: 6 }}>
                  ...
                  <input
                    ...
                    style={{ paddingLeft: 26, fontSize: 12, borderRadius: 7 }}
                    placeholder="Search or type guild ID…"
                    value={pickerSearch}
                    onChange={e => setPickerSearch(e.target.value)}
                    onKeyDown={e => {
                      if (e.key === 'Enter' && pickerSearch.trim()) {
                        setGuildId(pickerSearch.trim());
                        setPickerSearch('');
                        setPickerOpen(false);
                      }
                    }}
                ...
                <div style={{ overflowY: 'auto', flex: 1 }}>
                  {filteredGuilds.length === 0 && pickerSearch && (
                    <button onClick={() => { setGuildId(pickerSearch.trim()); setPickerSearch(''); setPickerOpen(false); }}
                      ...
                      Use "{pickerSearch}" as guild ID →
                    </button>
                  )}
                  {filteredGuilds.map(g => (
                    <button key={g.guild_id}
                      onClick={() => { setGuildId(g.guild_id); setPickerSearch(''); setPickerOpen(false); }}
                      style={{
                        width: '100%', padding: '8px 10px', borderRadius: 7, border: 'none',
                        background: g.guild_id === guildId ? 'var(--primary-subtle)' : 'transparent',
                        color: g.guild_id === guildId ? '#818cf8' : 'var(--text)',
                        cursor: 'pointer', textAlign: 'left', marginBottom: 2,
                      }}>
                      <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {g.guild_id}
                      </div>
                      <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'Lexend, sans-serif' }}>
                        {g.source} · {g.count} rows
                      </div>
                    </button>
                  ))}
                  {filteredGuilds.length === 0 && !pickerSearch && (
                    <div style={{ padding: '12px 10px', color: 'var(--text-muted)', fontSize: 12, textAlign: 'center' }}>
                      No guilds found in DB
                    </div>
                  )}
                </div>
          ...
          {guilds.length > 0 && (
            <div style={{ fontSize: 10, color: 'var(--text-faint)', marginTop: 5, paddingLeft: 2 }}>
              {guilds.length} guild{guilds.length !== 1 ? 's' : ''} found across database
            </div>
          )}
        </div>
      </aside>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', lineHeight: 1 }}>{currentNav.label}</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {guildId && (
              <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: 'var(--text-faint)', background: 'var(--elevated)', padding: '4px 10px', borderRadius: 6, border: '1px solid var(--border)' }}>
                {guildId}
              </div>
            )}
            <div title="Connected to NeonDB" style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: 'var(--success)' }}>
