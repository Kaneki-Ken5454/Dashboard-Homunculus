...
export interface Trigger { id: number; guild_id: string; trigger_text: string; response: string; match_type: string; response_type: string; enabled: boolean; cooldown_seconds: number; permission_level: string; channel_id?: string | null; delete_message: boolean; embed_color?: string | null; embed_title?: string | null; use_count: number; created_at: string; updated_at?: string; }
...
export interface InfoTopic { id: number; guild_id: string; section: string; subcategory?: string; topic_id: string; name: string; embed_title?: string; embed_description?: string; emoji?: string; embed_color?: string; image?: string; thumbnail?: string; footer?: string; category_emoji_id?: string; views: number; created_at: string; updated_at: string; }
...
export async function getDashboardStats(guildId: string) {
  return apiCall<{ memberCount: number; commandCount: number; ticketCount: number; auditCount: number; triggerCount: number; warnCount: number; autoRespCount: number; voteCount: number; topicCount: number; violationCount: number }>('getDashboardStats', { guildId });
}
...
export async function createVote(d: { guild_id: string; question: string; options: unknown; channel_id?: string; duration_minutes?: number; anonymous?: boolean }) {
  return apiCall('createVote', { guildId: d.guild_id, question: d.question, options: d.options, channelId: d.channel_id, durationMinutes: d.duration_minutes ?? 1440, anonymous: d.anonymous ?? false });
}
export async function deleteVote(id: string | number) {
  return apiCall('deleteVote', { id: String(id) });
}
